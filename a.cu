...
struct SuperReal2{
    float real;
    float zeta; // zeta^2 = 0
    SuperReal2(float a, float b):real(a),zeta(b){}
    SuperReal2(float a):real(a),zeta(1){}
    SuperReal2 operator+(const SuperReal2& a) const{
...
int main(){
    ...
    display.add_on_update([&](CurrentScreenInputInfo& info){
        ...
        BENCHMARK(main_tensor = main_tensor + tensor;)
        BENCHMARK(main_tensor = main_tensor * main_tensor;)

        
        auto main_tensor_cpu =  main_tensor.to(DeviceType::kCUDA);

        auto reshaped = main_tensor_cpu.view<float,3>({size, horizontal_size, 4});
        
        auto reshapedn = reshaped * reshaped;
        auto distance = min(pow(reshapedn[{{},{},0}] + reshapedn[{{},{},1}] + reshapedn[{{},{},2}] + reshapedn[{{},{},3}],0.5f)/float(4.0f/255.0f),255.0f);

        display[{{}}] = 0x00000000;

        // auto cpu_view = distance.to(DeviceType::kCPU);

        display.view<uint8_t,3>({size,horizontal_size,4})[{{},{},0}] = distance; // R


        // display.view<uint8_t,3>({size,horizontal_size,4})[{{},{},2}] = (uint8_t)0xff; // Alpha channel
        // display.view<uint8_t>({size,horizontal_size,4})[{{},{},0}] = uint8_t(0xff); // R
        // display.view<uint8_t,3>({size,horizontal_size,4})[{{},{},1}] = distance;
